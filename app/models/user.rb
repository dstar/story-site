class User < ActiveRecord::Base
  ...
  def has_story_permission(story,permission)
    ...
    @story_permissions[story_id] = self.story_permissions.find(:conditions => "story_id = #{story_id}") if @story_permissions[story_id].empty?
    ...
    unless obtained_permisson
      self.groups.each do |group|
        ...
        @group_story_permissions[group][story_id] = group.story_permissions.find(:conditions => "story_id = #{story_id}") if @group_story_permissions[group][story_id].empty?
  ...
  def has_universe_permission(universe,permission)
    ...
    @universe_permissions[universe_id] = self.universe_permissions.find(:conditions => "universe_id = #{universe_id}") if @universe_permissions[universe_id].empty?
    ...
    unless obtained_permisson
      self.groups.each do |group|
        ...
        @group_universe_permissions[group][universe_id] = group.universe_permissions.find(:conditions => "universe_id = #{universe_id}") if @group_universe_permissions[group][universe_id].empty?
