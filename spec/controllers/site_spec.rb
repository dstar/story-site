...
describe "Site" do
  ...
  describe "#show" do
    it "should allow unauthed requests" do
      ...
      response = request("http://playground.playground.pele.cx/", :method => "GET")
    ...
    it "should respond correctly" do
      response = request("http://playground.playground.pele.cx/", :method => "GET")
      response.should have_xpath("//a[@href='http://jaknis.playground.pele.cx/']")
    end
  ...
  describe "#new_universe" do
    ...
    it "Should not allow unauthed requests" do
      response = request("http://playground.playground.pele.cx/site/new_universe/", :method => "GET")
  ...
  describe "#create_universe" do
    ...
    it "should not allow unauthed creates" do
      request("http://playground.playground.pele.cx/logout", :method => "GET")
      lambda { request("http://playground.playground.pele.cx/site/create_universe/", :method => "POST", :params => {:universe => { :name => "test", :description => "test"}})}.should_not change(Universe,:count)
    end

    it "should allow authed creates" do
      request("http://playground.playground.pele.cx/login", :method => "PUT", :params => { :username => 'dstar', :password => 'test password' })
      lambda { request("http://playground.playground.pele.cx/site/create_universe/", :method => "POST", :params => {:universe => { :name => "test", :description => "test"}}) }.should change(Universe,:count)
    end
  end

  describe "#universe_add_owner" do
    it "should not allow unauthed owner additions" do
      unauthed_action(Site, :universe_add_owner, 1, "GET").should redirect_to("http://playground.playground.pele.cx/")
    end
  ...
  describe "#universe_owner_add_save" do
    it "should not allow unauthed owner addition saves" do
      unauthed_action(Site, :universe_owner_add_save, nil, "POST",  {:universe_id => 1, :type => 'user', :permission_holder => "Velvet Wood", :permission => "test" }).should redirect_to("http://playground.playground.pele.cx/")
    end
  ...
  describe "#expire_cache" do
    ...
    it "should not allow unauthed expires" do
      ...
      unauthed_action(Site, :expire_cache, nil).should redirect_to("http://playground.playground.pele.cx/")
      Merb::Cache[:default].read("expire_test").should_not be_nil
    end
    ...
    it "should allow authed expires" do
      ...
      Merb::Cache[:default].read("expire_test").should_not be_nil
      authed_action(Site, :expire_cache, nil).should redirect_to("http://playground.playground.pele.cx/")
